...
namespace nav
{
    /// template class that represents the underlying tree data structure for RRT-like planning algorithms
    /**
     * @tparam T               Data type
     */
    template <typename T>
    ...
    public:
        ...
        public:
            ...
            virtual Derived & operator++(int x) = 0;

            TreeNode<T>* operator->(){return m_ptr;}
        ...
        public:
            ...
            BFSIterator& operator++(int x) override
            {
        ...
        public:
            ...
            DFSIterator& operator++(int x) override
            {
    ...
    public:
        ...
        {
            std::vector<T> dist = m_nnFinder->m_queryMetric->distance(parent->getState(), state); // compute distance first, if it throws exception, the rest will not be executed.
        ...
        bool insertNodeByID(std::size_t idx, const std::vector<T>& state, std::size_t& idxNew)
        {
            idxNew = 0;
        ...
        int rewireNodeByID(std::size_t nodeID, std::size_t newParentNodeID)
        {
            if (nodeID < m_nodePtrs.size() && newParentNodeID < m_nodePtrs.size())
            {
                ...
                // normal case: node's parent is not newParentNode, and newParentNode is not a descendant of node. 
                pNode->m_parent->removeChild(pNode);
                pNewParentNode->addToChildren(pNode);

                // propagate cost changes downstream
                auto newDist = computeDistanceBetweenNodes(pNewParentNode, pNode);
        ...
        bool isDescendantOf(TreeNode<T>* pNode1, TreeNode<T>* pNode2)
        {
            bool result = false;
        ...
        {
            std::vector<T> dist = m_nnFinder->m_queryMetric->distance(node1->getState(), node2->getState());
            return dist[0];
        }
        ...
        /// Returns the IDs of the nodes within the closed ball of radius r centered at the given state. r is computed adaptively.
        ...
        T computeBallRadius()
        {
            ...
            T d = std::pow(m_ballRadiusConstant * std::log(numNodes) / numNodes, static_cast<T>(1.0) / m_nodeDim);
