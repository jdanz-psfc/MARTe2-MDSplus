...
namespace nav
{
    ...
    public:
        ...
        ExhaustiveNN(std::size_t dim) : NearestNeighborFinder<T>(dim), m_nodePtrs {} { /*ExhaustiveNN constructor*/ };
        ...
        std::vector<TreeNode<T>*> near(const std::vector<T>& state, T radius) override
        {
            ...
            std::sort(distPairs.begin(), distPairs.end(), [](const std::pair<T, TreeNode<T>*> &a,
                const std::pair<T, TreeNode<T>*> &b) { return a.first < b.first; });

            // retrieve those with distance smaller than radius
            auto lbIt = std::lower_bound(distPairs.begin(), distPairs.end(), radius,
                [](const std::pair<T, TreeNode<T>*> &a, T val) -> bool
                  { return a.first < val; });
