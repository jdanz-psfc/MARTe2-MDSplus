...
namespace nav
{
    ...
    public:
        ...
        CommonCSMetric(std::size_t dim, const std::vector<int>& topologies, const std::vector<T>& weights)
        {
            this->m_dim = dim;
        ...
        void configure(const std::vector<int>& topologies, const std::vector<T>& weights)
        {
            assert(topologies.size() == this->m_dim);
            assert(weights.size() == this->m_dim);
            m_topologies = topologies;
    ...
    protected:
        ...
        T distanceInternal(const typename std::vector<T>::const_iterator & state1It, const typename std::vector<T>::const_iterator & state2It)
        {
            ...
            for (std::size_t k = 0; k < this->m_dim; k++)
            {
                ...
                {
                case Topology::CIRCLE: /*S1 circle, i.e. 2D rotation*/
                    t = std::fabs(wrapToPi(*(state2It + k)) - wrapToPi(*(state1It + k)));
                    dist = std::fmin(t, 2 * M_PI - t);
                    break;

                case Topology::REALPROJECTIVE: /* quaternion, [k, k+3] */
                    dotProd = unitQuatDotProduct(state1It, state2It, k);
                    dist = std::fmin(std::acos(dotProd), std::acos(-dotProd)); // angle between two unit quaternions
                    k += 3; // RP3 (quaternion) always has 4 (3+1) elements, another k addition will happen as part of the for-loop
                    break;

                case Topology::EUCLIDEAN: /*Euclidean 1-space*/
                default:
        ...
        T unitQuatDotProduct(const typename std::vector<T>::const_iterator& state1It, const typename std::vector<T>::const_iterator& state2It, std::size_t idxStart)
        {
            T dotProd{}, norm1{}, norm2{};
        ...
        T wrapToPi(T inputRadians)
        {
            ...
            if (!std::signbit(x) /* true if negative*/ && (x == 0)) // positive multiples of 2 * pi map to 2 * pi
            {
                x = 2 * M_PI;
            }
            return x - M_PI;
