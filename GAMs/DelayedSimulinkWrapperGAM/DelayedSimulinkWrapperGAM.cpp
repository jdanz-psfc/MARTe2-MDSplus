...
#include "DelayedSimulinkWrapperGAM.h"
...
DelayedSimulinkWrapperGAM::DelayedSimulinkWrapperGAM()
        : GAM()
        , MessageI() {
...
bool DelayedSimulinkWrapperGAM::Initialise(StructuredDataI &data) {
 
    bool status = false;

if (initCount == 0u) {
    status = GAM::Initialise(data);
    printf("\n\nINIT %s PRIMO PEZZO\n\n", symbolPrefix.Buffer());
    data.Copy(dataCopy);
...
bool DelayedSimulinkWrapperGAM::Setup() {
...
bool DelayedSimulinkWrapperGAM::SetupSimulink() {
...
bool DelayedSimulinkWrapperGAM::Execute() {
...
/*lint -e{613} NULL pointers are checked beforehand.*/
bool DelayedSimulinkWrapperGAM::ScanTunableParameters(const rtwCAPI_ModelMappingInfo* const mmi)
{
...
bool DelayedSimulinkWrapperGAM::ScanParametersStruct(const uint32 dataTypeIdx, const uint32 depth, void* const startAddress, StreamString baseName, const uint64 baseOffset, StreamString spacer) {
...
/*lint -e{613} NULL pointers are checked in the caller method.*/
bool DelayedSimulinkWrapperGAM::ScanParameter(const uint16 parIdx, const uint32 depth, const ParameterMode mode, void* const startAddress, StreamString baseName, const uint64 baseOffset, StreamString spacer)
{
    /*lint --e{ 923, 9016, 9091 } pointer arithmetic in this method looks safe. However, that part should probably be refactored */
    ...
    switch(mode) {
        ...
        default:
            REPORT_ERROR(ErrorManagement::FatalError, "Wrong parameter mode in DelayedSimulinkWrapperGAM::ScanParameter(), depth %u", depth);
            ok = false;
...
/*lint -e{613} NULL pointers are checked beforehand.*/
bool DelayedSimulinkWrapperGAM::ScanRootIO(const rtwCAPI_ModelMappingInfo* const mmi, const SignalDirection mode) {
    /*lint --e{ 923, 9016, 9091 } pointer arithmetic in this method looks safe. However, that part should probably be refactored */
    ...
    if (ok) {
        ...
        default:
            sigGroup = NULL_PTR(rtwCAPI_Signals*);
            REPORT_ERROR(ErrorManagement::FatalError, "Wrong signal direction in DelayedSimulinkWrapperGAM::ScanRootIO()");
            break;
        }
    ...
    for (uint16 sigIdx = 0u; (sigIdx < nOfSignals) && ok ; sigIdx++) {
        ...
        switch(mode)
        {
            ...
            default:
            currentPort = NULL_PTR(SimulinkPort*);
                REPORT_ERROR(ErrorManagement::FatalError, "Wrong mode in DelayedSimulinkWrapperGAM::ScanRootIO()");
                ok = false;
        }
...
/*lint -e{613} NULL pointers are checked in the caller method.*/
bool DelayedSimulinkWrapperGAM::ScanSignalsStruct(const uint32 dataTypeIdx, const uint32 depth, void* const startAddress, StreamString baseName, const uint64 baseOffset, StreamString spacer){
...
/*lint -e{613} NULL pointers are checked in the caller method.*/
bool DelayedSimulinkWrapperGAM::ScanSignal(const uint16 sigIdx, const uint32 depth, const SignalMode mode, void* const startAddress, StreamString baseName, const uint64 baseOffset, StreamString spacer) {
    /*lint --e{ 923, 9016, 9091 } pointer arithmetic in this method looks safe. However, that part should probably be refactored */
    ...
    switch (mode) {
        ...
        default:
            REPORT_ERROR(ErrorManagement::FatalError, "Wrong signal mode in DelayedSimulinkWrapperGAM::ScanSignal(), depth %u", depth);
            ok = false;
    }
...
bool DelayedSimulinkWrapperGAM::CheckrtwCAPITypeAgainstSize(StreamString cTypeName, const uint16 checkSize) const
{
    return ( GetTypeSizeFromCTypeName(cTypeName.Buffer()) == checkSize );
}

void DelayedSimulinkWrapperGAM::PrintAlgoInfo() const {
...
bool DelayedSimulinkWrapperGAM::MapPorts(const SignalDirection direction) {
...
CLASS_REGISTER(DelayedSimulinkWrapperGAM, "1.0")
